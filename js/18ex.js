...
// members 배열에 저장된 객체의 name 속성값 출력=>'김사나' 박모모 이다현
...
//2)객체의 address 속성값이 '서울'인 name
for (let m of members) {
    if (m.address === '서울') {
        console.log('서울 거주 :', m.name)
    }
}
//3)객체 배열에서 age 25 이상인 name 속성값 출력

for (let m of members) {
    if (m.age >= 25) {
        console.log('25세이상:', m.name)
    }
}
...
for (let m of members) {
    console.log('-------')
    for (let key in m) {  //객체 m 의 모든 속성을 순서대로 가져와서 key 에저장 
        console.log(m[key])
    }
...
// 위에 코드를 화살표 함숭로도 변경 가능 
members.forEach( //하나씩 가져온 요소를 item 저장
    (item) => {
